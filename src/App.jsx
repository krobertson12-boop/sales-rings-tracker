...
function App() {
  ...
  // Add credit apps to weekly targets (5x daily target)
  weeklyTargets['credit_apps'] = (targets['credit_apps'] || DEFAULT_TARGETS['credit_apps']) * 5
  ...
  const handleSettingChange = (rep, metric, value) => {
    // Get current targets for this rep
    const currentTargets = getTargetsForRep(rep)
    // Initialize edits with all current values if not already done
    if (!settingsEdits[rep]) {
      settingsEdits[rep] = { 
        meetings_set: currentTargets.meetings_set,
        meetings_ran: currentTargets.meetings_ran,
        opportunities: currentTargets.opportunities,
        credit_apps: currentTargets.credit_apps,
        lasers_sold: currentTargets.lasers_sold
      }
    }
    // Update the specific metric
    settingsEdits[rep][metric] = value ? parseInt(value) : currentTargets[metric]
  ...
  const saveSettings = async () => {
    try {
      ...
      for (const [rep, edits] of Object.entries(settingsEdits)) {
        const toSave = {
          setting_type: 'rep_targets',
          rep_name: rep,
          value: JSON.stringify({
            meetings_set: edits.meetings_set,
            meetings_ran: edits.meetings_ran,
            opportunities: edits.opportunities,
            credit_apps: edits.credit_apps,
            lasers_sold: edits.lasers_sold
          })
        }
        
        console.log('Saving for', rep, toSave)
        ...
        if (error) {
          console.error('Error saving for', rep, error)
          throw error
        }
      }
      
      setSettingsEdits({})
      await new Promise(r => setTimeout(r, 500)) // Wait a moment
      await loadTargets()
      alert('Targets saved!')
    } catch (err) {
                ...
                {(() => {
                  ...
                  return (
                    <div style={{marginBottom:'10px'}}>
                      <label style={{fontSize:'11px',color:'#666',display:'block',marginBottom:'4px'}}>Credit apps</label>
                      <input
                        type="number"
                        min="1"
                        value={current || ''}
