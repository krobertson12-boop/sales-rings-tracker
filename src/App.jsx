...
const CHART_COLORS = ['#1a56db', '#059669', '#7c3aed', '#d97706', '#db2777', '#6b7280', '#f59e0b', '#10b981']
        ...
        if (entries) {
          ...
          REPS.forEach(rep => {
            ...
            repEntries.forEach(entry => {
              ...
              runningMeetings += parseInt(entry.meetings_ran) || 0
            })
            
            weekData[rep] = {
              meetings_ran: runningMeetings,
      ...
      {currentView === 'analytics' && (
        <div id="view-analytics">
          <div style={{marginBottom:'2rem'}}>
            ...
            {['meetings_ran', 'opportunities', 'credit_apps', 'lasers_sold'].map(metricKey => {
              ...
              return (
                <div key={metricKey} style={{marginBottom:'2rem',background:'#fff',border:'0.5px solid #ddd',borderRadius:'12px',padding:'1rem'}}>
                  <div style={{fontSize:'12px',fontWeight:'500',color:'#666',marginBottom:'1rem'}}>{metric.label}</div>
                  
                  <svg width="100%" height="300" viewBox="0 0 600 300" style={{border:'0.5px solid #eee',borderRadius:'8px'}}>
                    {/* Grid */}
                    {[0,1,2,3,4].map(i => (
                      <line key={`h-${i}`} x1="40" y1={250-i*50} x2="580" y2={250-i*50} stroke="#f0f0f0" strokeWidth="1" />
                    ))}
                    {[0,1,2,3,4].map(i => (
                      <line key={`v-${i}`} x1={40+i*135} y1="0" x2={40+i*135} y2="260" stroke="#f0f0f0" strokeWidth="1" />
                    ))}
                    
                    {/* Y axis labels */}
                    {[0,50,100,150,200].map((label, i) => (
                      <text key={`y-${i}`} x="25" y={260-i*50} fontSize="10" textAnchor="end" fill="#999">{Math.round(label*100/200)}</text>
                    ))}
                    ...
                    {REPS.map((rep, repIdx) => {
                      ...
                      weeks.forEach((week, weekIdx) => {
                        ...
                        const x = 40 + (weekIdx * 135) + 25
                        const y = 260 - (val / 200) * 260
                        pathD += (weekIdx === 0 ? 'M' : 'L') + x + ' ' + y + ' '
                      })
                      
                      return (
                        <g key={`line-${repIdx}`}>
                          <path d={pathD} stroke={CHART_COLORS[repIdx % 8]} strokeWidth="2" fill="none" />
                          {weeks.map((week, weekIdx) => {
                            const val = parseFloat(chartData[week].data[rep][metricKey]) || 0
                            const x = 40 + (weekIdx * 135) + 25
                            const y = 260 - (val / 200) * 260
                            return <circle key={`dot-${weekIdx}`} cx={x} cy={y} r="3" fill={CHART_COLORS[repIdx % 8]} />
                          })}
                        </g>
                      )
                    })}
                    ...
                    {/* X axis labels */}
                    {weeks.map((week, idx) => (
                      <text key={`x-${idx}`} x={40 + (idx * 135) + 25} y="280" fontSize="10" textAnchor="middle" fill="#999">
                        {chartData[week].week}
                      </text>
                    ))}
                  </svg>
                  ...
                  <div style={{display:'flex',gap:'16px',marginTop:'12px',flexWrap:'wrap',fontSize:'11px'}}>
                    {REPS.map((rep, idx) => (
                      <div key={`leg-${idx}`} style={{display:'flex',alignItems:'center',gap:'6px'}}>
                        <div style={{width:'12px',height:'2px',background:CHART_COLORS[idx % 8]}}></div>
                        <span style={{color:'#666'}}>{rep}</span>
                      </div>
                    ))}
                  </div>
                </div>
