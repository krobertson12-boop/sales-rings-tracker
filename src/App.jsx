...
function App() {
  ...
  const handleSettingChange = (rep, metric, value) => {
    ...
    // Initialize edits with all current values
    if (!settingsEdits[rep]) {
      settingsEdits[rep] = { ...currentTargets }
    }
    // Update the specific metric
    settingsEdits[rep][metric] = value ? parseInt(value) : ''
  ...
  const saveSettings = async () => {
    try {
      for (const [rep, edits] of Object.entries(settingsEdits)) {
        // Merge edits with current values to ensure no fields are lost
        const current = getTargetsForRep(rep)
        const toSave = { ...current, ...edits }
