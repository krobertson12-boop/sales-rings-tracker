...
function App() {
      ...
      for (const week of weeks) {
        ...
        if (entries) {
          ...
          REPS.forEach(rep => {
            ...
            weekData[rep] = {
              opportunities: weekTotals.meetings_ran > 0 ? (weekTotals.opportunities / weekTotals.meetings_ran).toFixed(2) : 0,
              credit_apps: weekTotals.meetings_ran > 0 ? (weekTotals.credit_apps / weekTotals.meetings_ran).toFixed(2) : 0,
              lasers_sold: weekTotals.meetings_ran > 0 ? (weekTotals.lasers_sold / weekTotals.meetings_ran).toFixed(2) : 0,
  ...
  const handleLoginSubmit = () => {
    ...
    else if (hashPassword(loginPassword) === MANAGER_PASSWORD_HASH) {
      setIsManager(true)
  ...
  const getYAxisScale = (metricKey) => {
    ...
    else max = Math.ceil(maxVal / 10) * 10
    
    const step = max / 5
    const labels = []
    for (let i = 0; i <= 5; i++) {
      labels.push((step * i).toFixed(maxVal > 10 ? 0 : 1))
    }
    ...
    <div className="wrap">
          ...
          <div className="modal-content">
            ...
            {!loginRepName ? (
              <>
                ...
                <button
                  onClick={() => setLoginPassword('cynomanager')}
      ...
      <div className="topbar">
        ...
        <div className="view-tabs">
          ...
          {!isRepLoggedIn && !isManager && <button className={`vtab ${currentView === 'board' ? 'active' : ''}`} onClick={() => setCurrentView('board')}>Weekly board</button>}
          {!isRepLoggedIn && !isManager && <button className={`vtab ${currentView === 'analytics' ? 'active' : ''}`} onClick={() => setCurrentView('analytics')}>Analytics</button>}
          {isManager && <button className={`vtab ${currentView === 'board' ? 'active' : ''}`} onClick={() => setCurrentView('board')}>Weekly board</button>}
          {isManager && <button className={`vtab ${currentView === 'analytics' ? 'active' : ''}`} onClick={() => setCurrentView('analytics')}>Analytics</button>}
                  ...
                  {['opportunities', 'credit_apps', 'lasers_sold', 'lasers_aggregate', 'meetings_aggregate'].map(metricKey => {
                    ...
                    return (
                      <div key={metricKey} style={{marginBottom:'2rem',background:'#fff',border:'0.5px solid #ddd',borderRadius:'12px',padding:'1rem'}}>
                        ...
                        <svg width="100%" height="320" viewBox="0 0 650 320" style={{border:'0.5px solid #eee',borderRadius:'8px'}}>
                          {(() => {
                            ...
                            return (
                              <>
                                {/* Grid */}
                                {[0,1,2,3,4,5].map(i => (
                                  <line key={`h-${i}`} x1="40" y1={270-i*45} x2="620" y2={270-i*45} stroke="#f0f0f0" strokeWidth="1" />
                                ))}
                                {weeks.map((_, i) => (
                                  <line key={`v-${i}`} x1={40+i*145} y1="0" x2={40+i*145} y2="280" stroke="#f0f0f0" strokeWidth="1" />
                                ))}
                                
                                {/* Y axis */}
                                <line x1="40" y1="0" x2="40" y2="280" stroke="#ddd" strokeWidth="1" />
                                
                                {/* Y axis labels */}
                                {yScale.labels.map((label, i) => (
                                  <text key={`y-${i}`} x="32" y={273-i*45} fontSize="10" textAnchor="end" fill="#999">{label}</text>
                                ))}
                                ...
                                {REPS.map((rep, repIdx) => {
                                  ...
                                  weeks.forEach((week, weekIdx) => {
                                    const val = parseFloat(chartData[week].data[rep][metricKey]) || 0
                                    const x = 40 + (weekIdx * 145) + 30
                                    const y = 270 - (Math.min(val, yScale.max) / yScale.max) * 270
                                    pathD += (weekIdx === 0 ? 'M' : 'L') + x + ' ' + y + ' '
                                    points.push({ x, y, val, rep, weekIdx })
                                  })
                                  
                                  return (
                                    <g key={`line-${repIdx}`}>
                                      <path d={pathD} stroke={CHART_COLORS[repIdx % 8]} strokeWidth="2.5" fill="none" />
                                      {points.map((p, idx) => (
                                        <circle key={`dot-${idx}`} cx={p.x} cy={p.y} r="3.5" fill={CHART_COLORS[repIdx % 8]} stroke="#fff" strokeWidth="1" />
                                      ))}
                                    </g>
                                  )
                                })}
                                
                                {/* Right side labels - rep names at their end points */}
                                {REPS.map((rep, repIdx) => {
                                  const lastWeek = weeks[weeks.length - 1]
                                  const val = parseFloat(chartData[lastWeek].data[rep][metricKey]) || 0
                                  const yScale = getYAxisScale(metricKey)
                                  const x = 40 + ((weeks.length - 1) * 145) + 30
                                  const y = 270 - (Math.min(val, yScale.max) / yScale.max) * 270
                                  return (
                                    <text key={`label-${repIdx}`} x={x + 12} y={y + 4} fontSize="11" fontWeight="500" fill={CHART_COLORS[repIdx % 8]}>
                                      {rep}
                                    </text>
                                  )
                                })}
                                ...
                                {/* X axis labels */}
                                {weeks.map((week, idx) => (
                                  <text key={`x-${idx}`} x={40 + (idx * 145) + 30} y="300" fontSize="11" textAnchor="middle" fill="#666" fontWeight="500">
                                    {chartData[week].week}
                                  </text>
                                ))}
                              </>
      ...
      {currentView === 'settings' && isManager && (
        <div id="view-settings">
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'1rem'}}>
            <div style={{fontSize:'13px',color:'#666',fontWeight:'500'}}>Set daily targets per rep (blank = use default)</div>
