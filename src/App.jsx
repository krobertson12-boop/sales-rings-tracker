...
function App() {
  ...
  const renderChart = (metricKey, metricLabel) => {
    ...
    return (
      <div key={metricKey} style={{marginBottom:'2rem',background:'#fff',border:'0.5px solid #ddd',borderRadius:'12px',padding:'1rem'}}>
        <div style={{fontSize:'12px',fontWeight:'500',color:'#666',marginBottom:'1rem'}}>{metricLabel}</div>
        
        <svg width="100%" height="320" viewBox="0 0 650 320" style={{border:'0.5px solid #eee',borderRadius:'8px'}}>
          {/* Grid */}
          {[0,1,2,3,4].map(i => (
            <line key={`h-${i}`} x1="40" y1={270-i*54} x2="620" y2={270-i*54} stroke="#f0f0f0" strokeWidth="1" />
          ))}
          {weeks.map((_, i) => (
          ...
          {[0,1,2,3,4,5].map((label, i) => (
            <text key={`y-${i}`} x="32" y={273-i*54} fontSize="10" textAnchor="end" fill="#999">{label}</text>
          ))}
          ...
          {REPS.map((rep, repIdx) => {
            ...
            weeks.forEach((week, weekIdx) => {
              ...
              const y = 270 - (Math.min(val, 5) / 5) * 270
      ...
      {currentView === 'analytics' && (
        <div id="view-analytics">
          <div style={{marginBottom:'2rem'}}>
            ...
            {renderChart('opportunities', 'Meetings Ran → Opportunities')}
            {renderChart('credit_apps', 'Meetings Ran → Credit Apps')}
            {renderChart('lasers_sold', 'Meetings Ran → Lasers Sold')}
            {renderChart('lasers_aggregate', 'Total Lasers Sold')}
          </div>
        </div>
      )}
