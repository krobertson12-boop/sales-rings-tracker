  ...
  const getYAxisScale = (metricKey) => {
    ...
    weeks.forEach(week => {
      REPS.forEach(rep => {
        ...
        // Only consider non-zero values for scaling
        if (val > 0 && val > maxVal) maxVal = val
      })
    })
    
    // If all values are 0, default to 1
    if (maxVal === 0) maxVal = 1
    
    // Round up to nice number
    let max = maxVal
    const multiplier = Math.pow(10, Math.floor(Math.log10(maxVal)))
    max = Math.ceil((maxVal / multiplier) * 2) * (multiplier / 2)
    
    const step = max / 5
    const labels = []
    for (let i = 0; i <= 5; i++) {
      labels.push((step * i).toFixed(maxVal > 10 ? 0 : 2))
    }
                          ...
                          {(() => {
                            ...
                            return (
                              <>
                                ...
                                {/* Y axis right */}
                                <line x1="630" y1="0" x2="630" y2="280" stroke="#ddd" strokeWidth="1" />
                                ...
                                {/* Y axis labels right */}
                                {yScale.labels.map((label, i) => (
                                  <text key={`y-right-${i}`} x="645" y={273-i*45} fontSize="10" textAnchor="start" fill="#999">{label}</text>
                                ))}
